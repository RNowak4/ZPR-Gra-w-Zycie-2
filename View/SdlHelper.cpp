...
void SdlHelper::draw(const SDL_Rect& camera, std::shared_ptr<SDL_Texture> tex, int x, int y, bool centered, double angle, Uint8 alpha)
{
	SDL_Rect tmp{ x, y };
...
void SdlHelper::renderText(const SDL_Rect& camera, std::shared_ptr<TTF_Font> font, const std::string& message, int x, int y, const SDL_Color& color)
{
...
void SdlHelper::drawFrame(const SDL_Rect& camera, SDL_Rect rectangle, std::shared_ptr<SDL_Texture> filling)
{
	/*Check if object is within camera bounds.*/
...
void SdlHelper::drawPoint(const SDL_Rect& camera, int x, int y, const SDL_Color & color)
{
